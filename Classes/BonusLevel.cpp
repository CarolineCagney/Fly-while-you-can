...
cocos2d::PhysicsWorld * sceneWorld2;
...
Scene* BonusLevel::createScene()//create scene
{
...
bool BonusLevel::init()//initing the game so the scene can be made 
{
	//////////////////////////////
	// 1. super init first
	if (!CCLayer::init())
	...
	this->schedule(schedule_selector(BonusLevel::addCross), 1.2);
	this->schedule(schedule_selector(BonusLevel::addGarlic), 1);
	this->schedule(schedule_selector(BonusLevel::addStake), 0.5);
	this->schedule(schedule_selector(BonusLevel::addBlood), 0.20);
	...
	auto MainMenu = MenuItemImage::create("MainMenu.png", "MainMenu.png", CC_CALLBACK_1(BonusLevel::GoToMainMenuScene, this));
	...
	contactListener->onContactBegin = CC_CALLBACK_1(BonusLevel::onContactBegin, this);
...
void BonusLevel::addCross(float dt)
{
...
void BonusLevel::addGarlic(float dt)
{
...
void BonusLevel::addStake(float dt)
{
	//making the enemy 
...
void BonusLevel::addBlood(float dt)
{
	//making the enemy 
...
bool BonusLevel::onTouchBegan(Touch * touch, Event *unused_event)
{
	// 2
...
bool BonusLevel::onContactBegin(PhysicsContact &contact)
{
	...
	*/
		...
		if ((2 == a->getCollisionBitmask() && 4 == b->getCollisionBitmask() || 4 == a->getCollisionBitmask() && 2 == b->getCollisionBitmask()))
	{
		...
		CCLOG("COLLISION HAS OCCURED");
			...
			if ((2 == a->getCollisionBitmask() && 5 == b->getCollisionBitmask() || 5 == a->getCollisionBitmask() && 2 == b->getCollisionBitmask()))
	{
		auto origin = Director::getInstance()->getVisibleOrigin();//setting up the origin 
		auto winSize = Director::getInstance()->getVisibleSize();// as well as the window size or the visible size as well 
		...
		__String * tempScore = __String::createWithFormat("%d", score3);
...
void BonusLevel::SetIsScored()
{
	scored = true;
}

bool BonusLevel::GetIsScored()
{
	return scored;
}

int BonusLevel::getScore()
{
...
void BonusLevel::menuCloseCallback(Ref* pSender)// setting up the close button "quit"
{
...
void BonusLevel::GoToMainMenuScene(Ref *sender)//go to game scene method implementation 
{
	SimpleAudioEngine::getInstance()->stopBackgroundMusic(GAME_SOUND);
	CCLOG("HELLO");
	this->unschedule(schedule_selector(BonusLevel::addCross));
	auto scene = MainMenuScene::createScene();
	Director::getInstance()->replaceScene(TransitionFade::create(TRANSITION_TIME, scene));
}

void BonusLevel::GoToLoseScene(Ref *sender)//go to game scene method implementation 
{
	SimpleAudioEngine::getInstance()->playEffect(DEATH_EFFECT);
	SimpleAudioEngine::getInstance()->stopBackgroundMusic(GAME_SOUND);
	this->unschedule(schedule_selector(BonusLevel::addCross));
	this->unschedule(schedule_selector(BonusLevel::addGarlic));
	this->unschedule(schedule_selector(BonusLevel::addStake));
	auto scene = LoseScene::createScene();
	Director::getInstance()->replaceScene(TransitionFade::create(TRANSITION_TIME, scene));
}

void BonusLevel::GoToGameOverScene(Ref *sender)//go to game scene method implementation 
{
	SimpleAudioEngine::getInstance()->stopBackgroundMusic(GAME_SOUND);
	this->unschedule(schedule_selector(BonusLevel::addCross));
...
void BonusLevel::update(float delta)
{
	time -= delta;
	if (time <= 0) {
		time = 10000000;
